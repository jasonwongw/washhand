...
class photoenhance:
    ...
    def Horizontal_flip(self):
        ...
        for sample_image, sample_box,sample_id in zip(images, boxes,category_ids):
            transformed = transform(image=sample_image, bboxes=[list(sample_box)],category_ids=sample_id)
    ...
    def read_image(self):  # path='C:\Users\Administrator\Desktop\handdata\HandWashDataset\train_label'
        ...
        for f in files_point:  # 读取一张图片手坐标(三维坐标点)
            with open(path + f, 'r') as fid:
                ...
                c.append(dat[:1])
                path_img = '/tmp/pycharm_project_634/train/' +f[:-4] + '.jpg'
        ...
        box_features = np.array(train_y, dtype='float')  ## (?,4)
        ...
        print("Shape of flipped_train_boxes:", np.shape(flipped_train_boxes))
        ...
        print("Shape of ver_train_boxes:", np.shape(ver_train_id))
        ...
        print("Shape of blur_train_boxes:", np.shape(blur_train_id))
        ...
        print("Shape of shitf_train_boxes:", np.shape(shitf_train_id))
        ...
        print("Shape of gauss_train_boxes:", np.shape(gauss_train_id))
