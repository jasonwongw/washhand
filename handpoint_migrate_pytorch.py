...
class structure:
    ...
    def train(self,traindata,evaldata):
        ...
        torch.save(best_model, 'best_handbox.pth')
        print('训练完成')

    def xywhn2xyxy(self,x, w=320, h=320, padw=0, padh=0): # w,h是图片变换后的大小
    ...
    def bbox_YOLO2VOC(self,yolo_bbox):
        yolo_bbox = np.array(yolo_bbox, dtype=float)
        voc_bbox = self.xywhn2xyxy(yolo_bbox)
        return voc_bbox

    # 绘图 YOLO格式
    def visualize_bbox_YOLO(self,img, bbox, class_id, class_idx_to_name, thickness=2):
        # 转换为xyxy
        bbox = self.bbox_YOLO2VOC(bbox)
        bbox = list(bbox[0])
        x1, y1, x2, y2 = bbox
        ...
        class_name = class_idx_to_name[class_id[0]]
    ...
    def predict_show(self,mod,inp):
        ...
        train_x=torch.from_numpy(train_images.reshape(-1,1,320,320)) #pd->torch tensor and np->torch
        ...
        plt.clf()
        category_ids = [0] #分类
        category_id_to_name = {0: '大'}  #分类对应的文字动作
        im = self.visualize_bbox_YOLO(test_img, pred, category_ids, category_id_to_name)
        plt.imshow(im)
        pylab.show()
        # 关键点预测
